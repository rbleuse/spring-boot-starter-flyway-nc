...
```kotlin
plugins {
    id("org.springframework.boot") version "4.2.0-M1" // applies io.spring.dependency-management
}

extra["flyway.version"] = "13.0.0" // optional override; defaults to the BOM's pinned version
...
- **Spring Boot 4.2.0-M1+ is required.** Version 2.0 targets the Boot 4.2 autoconfiguration / `ConnectionDetails` APIs.
...
To build against a different Flyway version, pass `-PflywayVersion=<version>` (for example, `./gradlew build -PflywayVersion=13.0.0`).
